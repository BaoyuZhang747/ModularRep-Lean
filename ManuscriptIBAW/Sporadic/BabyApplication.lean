...
import ManuscriptIBAW.Sporadic.BabyNumerical
...
/-!
# The Baby Monster applications in Proposition 5.4
...
namespace ManuscriptIBAW.Sporadic.BabyTwo

/-- Numerical and structural sources, before requiring common roots. -/
abbrev SourceInputs (base : NamedBase.{u}) := ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierAcceleratedNamedInputsBabyTwo.Inputs base
...
def RootAgreement {base : NamedBase.{u}} (inputs : SourceInputs base) : Prop := by
  ...
  | @mk k K U X inputInst1 inputInst2 inputInst3 inputInst4 inputInst5 inputInst6 inputInst7 inputInst8 iota hinj R source small C cover hcover hkernel hsimple hnonabelian hOuter D T compatibility fieldSource namedBaseEquiv =>
      ...
      let W := ManuscriptIBAW.Sporadic.CanonicalNumericalWitness.witnessOfNumerical
        ...
        (ManuscriptIBAW.Sporadic.BabyNumerical.numericalBlockwiseAWC iota hinj R source small) compatibility fieldSource
...
theorem model_eq_retained {base : NamedBase.{u}} (inputs : Inputs base) :
    model inputs = ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierAcceleratedNamedInputsBabyTwo.model inputs.source := by
...
end ManuscriptIBAW.Sporadic.BabyTwo

namespace ManuscriptIBAW.Sporadic.BabyOdd

/-- Numerical and structural sources, before requiring common roots. -/
abbrev SourceInputs (base : NamedBase.{u}) (p : ℕ) := ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierAcceleratedNamedInputsBabyOdd.Inputs base p
...
def RootAgreement {base : NamedBase.{u}} {p : ℕ} (inputs : SourceInputs base p) : Prop := by
  ...
  | @mk k K X S inputInst1 inputInst2 inputInst3 inputInst4 inputInst5 inputInst6 inputInst7 inputInst8 inputInst9 inputInst10 iota hinj R C hpne q hq hkernel hs hna hOuter D T source compatibility lower fieldSource namedBaseEquiv =>
      let selectedCover : EllPrimeCoverSource p X := ellPrimeCover_of_fullCover_kernel_two iota.prime hpne q hq hs hna hkernel
      let W := ManuscriptIBAW.Sporadic.OriginalNumericalWitness.witnessOfNumerical
        iota hinj R C selectedCover (allInner_of_fullCover_of_outer_card_one q hq hs hna hOuter) D T
        (ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierJ4Numerical.numericalBlockwiseAWC iota hinj R source) compatibility lower fieldSource
      exact OriginalWitnessRoots iota R selectedCover C D T W
...
theorem model_eq_retained {base : NamedBase.{u}} {p : ℕ} (inputs : Inputs base p) :
    model inputs = ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierAcceleratedNamedInputsBabyOdd.model inputs.source := by
  rcases inputs with ⟨source, roots⟩
  cases source
  rfl
...
theorem complete {base : NamedBase.{u}} {p : ℕ} (inputs : Inputs base p) : target inputs := by
  ...
  | @mk k K X S inputInst1 inputInst2 inputInst3 inputInst4 inputInst5 inputInst6 inputInst7 inputInst8 inputInst9 inputInst10 iota hinj R C hpne q hq hkernel hs hna hOuter D T source compatibility lower fieldSource namedBaseEquiv =>
      ...
      let W := ManuscriptIBAW.Sporadic.OriginalNumericalWitness.witnessOfNumerical
        iota hinj R C selectedCover (allInner_of_fullCover_of_outer_card_one q hq hs hna hOuter) D T
        (ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierJ4Numerical.numericalBlockwiseAWC iota hinj R source) compatibility lower fieldSource
      exact of_original iota R selectedCover C D T W roots
...
/-- At odd primes, use the double cover with kernel of order two. -/
theorem cover_kernel_card {base : NamedBase.{u}} {p : ℕ} (inputs : Inputs base p) :
    Nat.card (model inputs).Cover.quotient.ker = 2 := by
  rcases inputs with ⟨source, roots⟩
  cases source
  assumption
...
end ManuscriptIBAW.Sporadic.BabyOdd
