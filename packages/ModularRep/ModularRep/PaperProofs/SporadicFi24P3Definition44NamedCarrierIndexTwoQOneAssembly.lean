...
import ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierIndexTwoEquivariantReplacement
...
namespace ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierIndexTwoQOneAssembly
...
theorem centerless_manuscript_qOne_intermediate_of_index_two
    ...
    ∀ (_ : ∀ phi : FaithfulIBr iota inj bs R routine,
      ...
      (_ : ∀ phi : FaithfulIBr iota inj bs R routine,
      Nonempty (PrimeRegularRootEmbedding p k K (ActualAutAmbient iota phi.val)))
      ...
      ∃ (e : FaithfulIBr iota inj bs R routine ≃ FaithfulWeight iota inj bs R routine)
        ...
        CenterlessCommonOutput iota inj R bs routine hcenter e ∧
        CenterlessCommonBlockOutput iota inj R bs routine hcenter := by
  let : Invertible (Fintype.card (Subgroup.center G) : k) := centerlessCardInvertible hcenter
  let : Fintype (ActualBlock (k := k) (X := G)) :=
    R.1.operations.ambientBlockData.fintypeBlock
  dsimp only
  intro ambient seed Bzero e0 hblock he0
  let inj := FDRepSimpleClassKZero.irreducibleBrauerCharacterInjectivity_of_rootEmbedding iota
  let bs := R.1.operations.ambientBlockData.blocks
  let routine : RoutineTransportInput iota inj bs R := ⟨⟩
  obtain ⟨e, hFamily, hIndependent, hFixed, hOrdinary⟩ :=
    family_ordinary_deduction iota inj bs R routine nu0
      (Or.inl (centerless_card_one hcenter)) e0 he0
  have hOld : CenterlessOrdinaryOutput iota inj bs R routine hcenter e hFamily.1 := by
    obtain ⟨Omega, hclass, hcovariance, hfixed⟩ := hOrdinary compatibility availability
    ...
    refine ⟨source, centerless_action_output iota inj bs R routine e hFamily phi.val V
      (hclass nu Q phi) hcenter, ?_⟩
    exact SporadicFi24P3Definition44NamedCarrierIndexTwoEquivariantReplacement.centerless_equivariant_replacement_of_index_two
      iota phi.val.val V (centerlessOmega iota inj bs R routine hcenter e)
      (centerlessOmega_equivariant iota inj bs R routine hcenter e hFamily.1)
      ((hclass nu Q phi).trans (centerlessOmega_at_original iota inj bs R routine hcenter e phi.val).symm)
      source hcenter hOuter
  have hAD3 := centerless_ad3_output_of_ordinary iota inj bs R routine hcenter e hFamily.1 hOld
  have hBlocks := matched_block_output_of_base iota inj R bs routine nu0
    (Or.inl (centerless_card_one hcenter)) e0 hblock e hIndependent compatibility
  have hQOne := same_map_qOne_of_block_preserving iota inj R bs routine e hBlocks.1
    availability compatibility Dzero Tzero Bzero
  have hCommon := centerless_common_output iota inj R bs routine hcenter e
    availability compatibility Dzero Tzero hQOne seed
  have hCommonBlocks := centerless_common_block_output iota inj R bs routine hcenter hOuter ambient
  exact ⟨e, hFamily, hIndependent, hFixed, hAD3, hBlocks, hQOne, hCommon, hCommonBlocks⟩

end ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierIndexTwoQOneAssembly
