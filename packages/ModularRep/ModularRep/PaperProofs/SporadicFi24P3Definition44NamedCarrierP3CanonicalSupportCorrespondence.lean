...
import ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierCanonicalDefectZeroCounts
...
signature; canonical normalizer support determines the radical in the other known fibre.
The block-preserving map and its original Q=1 normalization are outputs. -/

noncomputable section
set_option maxHeartbeats 4000000
open scoped MonoidAlgebra
namespace ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierP3CanonicalSupportCorrespondence
...
open SporadicFi24P3Definition44NamedCarrierCanonicalDefectZeroCounts
...
include AD Bridge hOuter fixedNonprincipal fixedDefectZero ordinaryComplete ordinaryValues
  fusion support rowBijective S414 evaluation stable localFixed availability compatibility Bzero hDefect hdz

theorem exists_normalized_correspondence_from_canonical_support :
  ...
  let V3 := literalV3Binding encoding
  have htau := selected_outer_nontrivial_of_v3 encoding V3 (selected 4)
    (fun c => (representatives.representative c).1) (ordinaryValues 4) tau
    (by simpa only [regularOuterPermutation, Equiv.swap_apply_left] using fusion 27)
  ...
  obtain ⟨hBzero, hWzero, hBzeroFixed, hWzeroFixed⟩ :=
    selected_defectZero_signature_of_canonical_support iota (FDRepSimpleClassKZero.irreducibleBrauerCharacterInjectivity_of_rootEmbedding iota) R availability compatibility Dzero Tzero
      Bzero dz (roles 2) hDefect hdz tau fixedDefectZero
...
end ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierP3CanonicalSupportCorrespondence
