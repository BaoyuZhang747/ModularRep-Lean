import ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierIdentityFamily
...
namespace ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierCompleteCollapse
...
open ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierIdentityFamily
...
theorem exists_definition41_of_numerical
  ...
  let C := trivialWeightBlockCompatibilityOfOperations iota hinj R D T compatibility
  let TI := trivialWeightIdentification (K := K) T
  let Omega := globalEquiv (iota := iota) (hinj := hinj) (blocks := blocks)
    (R := R) (D := D) (T := T) (C := C) TI hAWC
  refine ⟨ofNormalizedEquiv iota hinj R localReduction Cover hc allInner.eq_conj D T
  ...
  · exact AllAutomorphismsInner.globalEquiv_equivariant
      ...
      (R := R) (D := D) (T := T) (C := C) (TI := TI) (hAWC := hAWC) allInner
  · intro phi
    exact (globalEquiv_block_preserving
      (iota := iota) (hinj := hinj) (blocks := blocks)
      (R := R) (D := D) (T := T) (C := C) (TI := TI) (hAWC := hAWC) phi).trans
      (operationsBlock_eq iota hinj R blocks phi).symm
  · exact globalEquiv_normalisation
      (iota := iota) (hinj := hinj) (blocks := blocks)
      (R := R) (D := D) (T := T) (C := C) (TI := TI) (hAWC := hAWC)

end ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierCompleteCollapse
