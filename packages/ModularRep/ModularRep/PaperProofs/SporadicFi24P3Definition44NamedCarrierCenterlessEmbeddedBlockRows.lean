...
namespace ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierCenterlessEmbeddedBlockRows
...
def CenterlessPositiveLocalInputs : Prop :=
  letI : Fact p.Prime := ⟨iota.prime⟩
  ∀ phi : FB,
    letI : Fintype (ActualAutAmbient iota phi.val) := Fintype.ofFinite _
    actualBase iota phi.val ≠ ⊤ →
      PositiveLocalCatalogueInput (p := p) (k := k) (innerEmbedding iota phi.val)

def CenterlessPositiveBlockOutput (hcenter : Subgroup.center G = ⊥) (e : FB ≃ FW) : Prop :=
  ∀ phi : FB,
    letI : Fintype (ActualAutAmbient iota phi.val) := Fintype.ofFinite _
    ∀ (V : CharacterWeight p K G),
      ...
      V.subgroup ≠ ⊥ → ∀ source : CanonicalRawReduction iota V,
        EveryEmbeddedModelPairBlocks
          ...
          (iota.alongMulEquiv (actualBaseEquiv iota phi.val hcenter))
          (source.normalizerRoot.alongMulEquiv (normalizerBaseEquiv (innerEmbedding iota phi.val)
            (innerEmbedding_injective iota phi.val hcenter) V.subgroup))
          (IrreducibleBrauerCharacter.alongMulEquiv iota (actualBaseEquiv iota phi.val hcenter) phi.val)
          (IrreducibleBrauerCharacter.alongMulEquiv source.normalizerRoot
            (normalizerBaseEquiv (innerEmbedding iota phi.val)
              (innerEmbedding_injective iota phi.val hcenter) V.subgroup) source.localBrauer)

theorem centerless_positive_block_output
...
end ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierCenterlessEmbeddedBlockRows
