import ModularRep.PaperProofs.TypeBRankThreePrincipalFullBlockAssembly
import ModularRep.PaperProofs.TypeBFixedBlockQuotientEquivariance

/-!
# The principal fixed-block condition on actual matrix Omega

The existing rank-three source telescope constructs the complete quotient
block correspondence, its actual block-stabilizer equivariance, and the
centrally faithful characters with their retained matched extensions.
Every character of the quotient block is tied to an original character and
the same stored local weight, ambient group, and intermediate block data.

The named application remains conditional on the explicit source inputs.
Its output is the independently defined fixed-block condition.
-/
noncomputable section
set_option autoImplicit false

namespace ModularRep.PaperProofs.TypeBRankThreePrincipalFixedBlockDecode
...
/-- Decode the rank-three principal construction on its actual identity cover. -/
theorem lemma_4_12_fixed_block_source_instantiated :
    Nonempty (TypeBFixedBlockDecodedCondition.DecodedCondition
  ...
  obtain ⟨W⟩ :=
    TypeBRankThreePrincipalFullBlockAssembly.exists_full_principal_block_witness
      (F := F)
      (K := K)
      (O := O)
      (k := k)
      ...
      (quotientBlockProvider := quotientBlockProvider)
      (targetBlockProvider := targetBlockProvider)
      (ksCertificate := ksCertificate)
      (localCertificate := localCertificate)
  exact ⟨TypeBFixedBlockQuotientEquivariance.fromBlockWitness W⟩

end Selected

end ModularRep.PaperProofs.TypeBRankThreePrincipalFixedBlockDecode
