...
/-! The named J4 input constructor stores only the checked application's
...
namespace ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierAcceleratedNamedInputsJ4
...
def realise {base : NamedBase.{u}} {p : ℕ} (I : Inputs base p) :
  ...
  cases I with
  | @mk k K U X inputInst1 inputInst2 inputInst3 inputInst4 inputInst5 inputInst6 inputInst7 inputInst8 inputInst9 iota hinj R C cover hcover hkernel hsimple hnonabelian hOuter D T source compatibility fieldSource namedBaseEquiv =>
      ...
      exact ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierAcceleratedCaseApplications.J4.definition41_from_external_inputs
        (p := p)
        (k := k)
        (K := K)
        (U := U)
...
end ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierAcceleratedNamedInputsJ4
