import ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierP3LocalDefectDefinition41
import ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierP3DerivedRadicalSupport

/-! The complete prime-three manuscript conclusion with role-one radical
support derived from its known defect, the canonical bottom-weight block, a
restricted order-three subgroup classification and actual quotient degree cover.
Every previous result and specified object is retained. -/

noncomputable section
set_option maxHeartbeats 4000000
open scoped MonoidAlgebra
namespace ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierP3RadicalSupportDefinition41
...
open SporadicFi24P3Definition44NamedCarrierP3DerivedRadicalSupport
...
variable (U : RadicalSubgroup (p := 3) (G := G))
variable (orderThreeClassification : ∀ P : RadicalSubgroup (p := 3) (G := G),
  P.1.IsSubconjugate Q.1 → Nat.card P.1 = 3 → P.1.AreConjugate U.1)
variable (quotientOrder : Nat.card (NormalizerQuotient U.1) = 29713078886400)
variable (quotientDegreeCover : ∀ chi : OrdinaryIrreducibleCharacter.Irr K (NormalizerQuotient U.1),
  ∃ r : Fin 125, chi 1 = (quotientDegrees r : K))

include AD Bridge hOuter ordinaryComplete ordinaryValues
  fusion rowSurjective stable hz probeValues imageProbeValues localFusion
  ...
  orderThreeClassification quotientOrder quotientDegreeCover

theorem p3_definition41_from_radical_support :
  ...
  have hZero : IsMaximalCentralBrauerDefect (p := 3)
    ...
    defect_bot_of_full_coefficient_table R.1.operations.ambientBlockData.blocks (roles 2)
      classRepresentatives classCover classCentralizers blockCoefficients
  have hRoleOne := role_one_has_defect iota R roles Dordinary C allocation hZero
    Q hQcard localDefectExponent localDefectOrder localExistence localUpper
    (rows 0) (localRowDefects 0)
  let Tzero : TrivialWeightSource (p := 3) (X := G) :=
    SporadicFi24P3Definition44NamedCarrierTrivialRadicalOfDefectZero.trivialWeightSourceOfDefectBot
      R.1.operations.ambientBlockData.blocks (roles 2) hZero
  let Dzero : DefectZeroReductionSource iota :=
    SporadicFi24P3Definition44NamedCarrierCanonicalGlobalDefectZeroReduction.defectZeroReductionSourceOfAvailability
      iota availability Tzero (regularRestriction_injective C)
  have support := weight_support_from_order_three_degree_cover iota R roles Dordinary C allocation
    Dzero Tzero availability compatibility Q hQcard hRoleOne.support411 U
    orderThreeClassification quotientOrder quotientDegreeCover
  exact SporadicFi24P3Definition44NamedCarrierP3LocalDefectDefinition41.p3_definition41_from_local_defect
    ...
    (hQcard := hQcard) (localDefectExponent := localDefectExponent)
    (localDefectOrder := localDefectOrder) (localRowDefects := localRowDefects)
    (localExistence := localExistence) (localUpper := localUpper)

end ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierP3RadicalSupportDefinition41
