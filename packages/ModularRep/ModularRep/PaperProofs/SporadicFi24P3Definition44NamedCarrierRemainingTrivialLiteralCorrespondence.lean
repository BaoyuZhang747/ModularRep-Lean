import ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierKleinFourLiteralCorrespondence
import ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierRemainingTrivialActualCounts
import Mathlib.Tactic.FinCases

/-! The original conditional correspondence with every trivial-sector Brauer
signature derived from literal ordinary values and specified allocation. -/

noncomputable section
open scoped MonoidAlgebra

namespace ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierRemainingTrivialLiteralCorrespondence
...
include T F L compatibility decomposition hcardCenter hinverts small hDefectTrivial
  hDefectFaithful D C A roots hvalues allocation fusion hBr25 hBrSmall

theorem exists_actual_equivariant_of_literal_trivial_block_values :
  ...
  have hD8 : actualBrauerSignature iota hinj blocks tau (trivialRoles 2).1 = (3, 3) := by
    ...
    · exact actualDihedralBlock_fixed_card_three_of_literal_values
        iota hinj blocks D C trivialRoles allocation A roots hvalues tau fusion
  have hZero : ∀ j : Fin 2,
    ...
    · exact actualZeroBlock_card_one_of_literal_values
        iota hinj blocks D C trivialRoles allocation A roots hvalues j
    · exact actualZeroBlock_fixed_card_one_of_literal_values
        iota hinj blocks D C trivialRoles allocation A roots hvalues j tau fusion
  have hBrRest : ∀ j : Fin 3,
      actualBrauerSignature iota hinj blocks tau (trivialRoles j.succ.succ).1 =
        nonprincipalSignature j.succ := by
    intro j
    fin_cases j
  ...
  exact exists_actual_equivariant_of_literal_trivial_and_kleinFour_values
    ...
    (hBrRest := hBrRest) (hBr25 := hBr25) (hBrSmall := hBrSmall)

end ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierRemainingTrivialLiteralCorrespondence
