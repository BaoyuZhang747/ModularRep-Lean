import ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierCanonicalBaseBlock
import ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierSpathSource

/-! The Späth matched-pair construction in one fixed original root
convention. The selected reductions are computed from the canonical raw
family. The local lift equation is retained from the extension constructor
and consumed internally to derive the specified base block relation. -/

noncomputable section

namespace ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierCanonicalSpathWitness
...
open ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierCanonicalBaseBlock
...
def canonicalSelectedReduction
  ...
  SporadicFi24P3Definition44NamedCarrierRawReduction.selectedLocalReduction R.1
    (fun V => (C V).toRaw)

theorem spathMatchedBlockCondition_of_canonical_sources
    (iota : PrimeRegularRootEmbedding p k K X)
  ...
  obtain ⟨localInflation, fixedLocal, initialGlobal, hselected, mkSource⟩ :=
    exists_cyclic_inputs_with_retained_lift P M S hcenter reference haut
      principle seed fieldSource catalogues
  let source := C (selectedCharacterWeight R.1 b M.weight)
  have hroot : localInflation.iota.lift = source.toRaw.quotientRoot.lift := hselected
  let base := baseBlockOfCanonicalOperations iota hinj R
    (canonicalSelectedReduction iota R C) b P rfl hcenter
    (characterExtensionsOfLocalAndGlobal fixedLocal initialGlobal)
    source hroot compatibility fieldSource
  obtain ⟨blockSource⟩ := mkSource base
  obtain ⟨extensions, ⟨allJ⟩⟩ := exists_extensions_and_intermediate_blocks
    S9295 S9495 S820 fixedLocal initialGlobal base blockSource
  exact ⟨{
...
structure CanonicalNamedMatchedPairWitness
...
theorem namedMatchedPairWitness_of_canonical_sources
    (iota : PrimeRegularRootEmbedding p k K X)
    (hinj : IrreducibleBrauerCharacterInjectivity iota)
    (R : LiteralCarrierAdapter (p := p) (k := k) (K := K) (X := X))
    (C : ∀ V : CharacterWeight p K X, CanonicalRawReduction iota V)
    ...
    (compatibility : CanonicalLocalBlockCompatibility P.iota P.blockSource.operations)
    (catalogues : BlockCatalogues P M S hcenter reference haut) :
    Nonempty (CanonicalNamedMatchedPairWitness P M S hcenter reference haut) := by
  obtain ⟨clause3⟩ := namedClause3Witness_of_equivariantMatch P M S hcenter haut principle
  obtain ⟨spath⟩ := spathMatchedBlockCondition_of_canonical_sources
    iota hinj R C b P hP M S hcenter reference haut principle
    S9295 S9495 S820 seed fieldSource compatibility catalogues
  exact ⟨{ clause3 := clause3, spath := spath }⟩

end ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierCanonicalSpathWitness
