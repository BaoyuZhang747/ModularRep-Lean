import ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierCenterlessOrdinaryAssembly
...
namespace ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierCenterlessAD3Assembly
...
def CenterlessProductMatchedPair (hcenter : Subgroup.center G = ⊥)
  ...
  CenterlessProductComparison iota phi.val hcenter V
    (centerlessOmega iota hinj blocks R E1 hcenter e)
    (centerlessOmega_equivariant iota hinj blocks R E1 hcenter e he)
    (hmatch.trans (centerlessOmega_at_original iota hinj blocks R E1 hcenter e phi).symm) source

def CenterlessAD3Output (hcenter : Subgroup.center G = ⊥)
  ...
  ∃ (Omega : ∀ (nu : S) (Q : RadicalSubgroup (p := p) (G := G)),
    ...
    ∀ (nu : S) (Q : RadicalSubgroup (p := p) (G := G))
      ...
      ∃ source : CanonicalRawReduction iota V,
        ...
        CenterlessActionOutput iota phi.val.val V hcenter ∧
        LocalOrdinaryRealization iota V source (normalizerBaseEquiv (innerEmbedding iota phi.val.val)
            (innerEmbedding_injective iota phi.val.val hcenter) V.subgroup) ∧
        CenterlessGammaData iota phi.val.val hcenter V (ordinaryCentralLambda iota nu.val) ∧
        CenterlessProductMatchedPair iota hinj blocks R E1 hcenter e he phi.val V (hclass nu Q phi) source

theorem centerless_ad3_output_of_ordinary (hcenter : Subgroup.center G = ⊥)
    (e : FB ≃ FW) (he : ∀ (a : A) (phi : FB), e (a • phi) = a • e phi)
    (hOld : CenterlessOrdinaryOutput iota hinj blocks R E1 hcenter e he) :
    CenterlessAD3Output iota hinj blocks R E1 hcenter e he := by
  ...
  refine ⟨source,
    ...
    local_ordinary_realization iota V source (normalizerBaseEquiv (innerEmbedding iota phi.val.val)
            (innerEmbedding_injective iota phi.val.val hcenter) V.subgroup), centerless_gamma_data iota phi.val.val hcenter V (ordinaryCentralLambda iota nu.val), ?_⟩
  apply centerless_product_comparison
  exact hComparison

omit [Invertible (Fintype.card (Subgroup.center G) : k)] in
/-- Full AD(3a-d), derived for every row of the same ordinary family. -/
theorem centerless_manuscript_ad3_assembly
    ...
    ∀ (e0 : Fibre bP nu0 ≃ Fibre wP nu0),
      ...
      ∃ (e : FaithfulIBr iota inj bs R routine ≃ FaithfulWeight iota inj bs R routine)
        ...
        CenterlessAD3Output iota inj bs R routine hcenter e hFamily.1 := by
  ...
  obtain ⟨e, hFamily, hIndependent, hFixed, hOld⟩ :=
    SporadicFi24P3Definition44NamedCarrierCenterlessOrdinaryAssembly.centerless_manuscript_ordinary_assembly iota R compatibility availability nu0 hcenter hOuter principle e0 he0
  exact ⟨e, hFamily, hIndependent, hFixed,
    centerless_ad3_output_of_ordinary iota
      (FDRepSimpleClassKZero.irreducibleBrauerCharacterInjectivity_of_rootEmbedding iota)
      R.1.operations.ambientBlockData.blocks R ⟨⟩ hcenter e hFamily.1 hOld⟩

end ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierCenterlessAD3Assembly
