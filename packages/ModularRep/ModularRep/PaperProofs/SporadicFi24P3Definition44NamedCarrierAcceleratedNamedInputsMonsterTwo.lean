...
/-! The named MonsterTwo input constructor stores only the checked application's
...
namespace ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierAcceleratedNamedInputsMonsterTwo
open ModularRep
...
inductive Inputs (base : NamedBase.{u}) : Type (u + 1) where
  | mk
    ...
    (namedBaseEquiv : (identityEllPrimeCover_of_fullCover_kernel_card_one iota.prime cover hcover hkernel hsimple hnonabelian).S ≃* base.S) : Inputs base
...
def realise {base : NamedBase.{u}} (I : Inputs base) :
  ...
  | @mk k K U X inputInst1 inputInst2 inputInst3 inputInst4 inputInst5 inputInst6 inputInst7 inputInst8 iota hinj R source small C cover hcover hkernel hsimple hnonabelian hOuter D T compatibility fieldSource namedBaseEquiv =>
      ...
      exact ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierAcceleratedCaseApplications.MonsterTwo.definition41_from_external_inputs
        (k := k)
        (K := K)
        (U := U)
        (X := X)
...
end ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierAcceleratedNamedInputsMonsterTwo
