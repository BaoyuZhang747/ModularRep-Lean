...
import ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierOriginalSourceHBlocks
import ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierFullCenterLift

/-!
# Spath Definition 4.1 with the original stabilizer and full centre

The original prime-to-p cover is an explicit index. One Omega
...
namespace ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierOriginalDefinition41
...
open ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierFullCenterLift
...
abbrev OriginalPacketAt
  ...
  OriginalActualWeightPacket
    (problemAt iota hinj R (canonicalSelectedReduction iota R C) xi.1)
    (ownBrauerAt iota hinj R (canonicalSelectedReduction iota R C) xi.1)
...
theorem localEquiv_centralLift
    ...
    ∃ thetaHat : OrdinaryIrreducibleCharacter.Irr K (Subgroup.normalizer (Q.1 : Set X)),
      ...
      ∀ nu : OrdinaryIrreducibleCharacter.Irr K (Subgroup.center X),
        (∀ z : PrimeRegularElement (G := Subgroup.center X) p,
          xi.1.1 (PrimeRegularElement.map (Subgroup.center X).subtype z) =
            xi.1.1 ⟨1, isPrimeRegular_one⟩ * nu z.1) →
        ∀ z : Subgroup.center X,
          thetaHat ⟨z.1, Subgroup.center_le_normalizer (Q.1 : Set X) z.2⟩ = thetaHat 1 * nu z := by
  ...
  let V := characterWeightAt iota.prime Q (W.localEquiv Q xi)
  apply exists_ordinary_lift_over_full_center P psi V (C V) W.compatibility ?_ Cover.centerPrimeTo
  change R.1.operations.rawWeightBlock V = operationsBlock iota hinj R xi.1
  exact (congrArg R.1.weightBlock (localMap_class iota W.Omega iota.prime Q xi)).trans
    (W.block_preserving xi.1)
...
theorem actualLocalBlockInducesTo
    (Q : RadicalSubgroup (p := p) (G := X)) (xi : BrauerAtRadical iota W.Omega Q) :
    let source := C (characterWeightAt iota.prime Q (W.localEquiv Q xi))
    let O := R.1.operations
    let localData := O.inflatedNormalizerBlockData Q.1
    letI := O.ambientBlockData.fintypeBlock
    letI := localData.fintypeBlock
    BlockInducesTo (Subgroup.normalizer (Q.1 : Set X))
      ...
      (NavarroLocalReductionInflationBlockCompatibility.normalizerBrauerBlock O Q.1
        source.normalizerRoot source.localBrauer)
  ...
  let V := characterWeightAt iota.prime Q (W.localEquiv Q xi)
  have h := W.compatibility.normalizerBrauerBlock_eq_inflateToNormalizer V (C V)
  dsimp only
...
/-- Actual local values are retained under the quotient normalizer map. -/
theorem packet_localBrauer_pullback_eq_canonical
    (Q : RadicalSubgroup (p := p) (G := X)) (xi : BrauerAtRadical iota W.Omega Q) :
    PrimeRegularClassFunction.pullback (W.packets Q xi).localMap
      (W.packets Q xi).localCharacter.1 =
        (C (characterWeightAt iota.prime Q (W.localEquiv Q xi))).localBrauer.1 := by
  apply PrimeRegularClassFunction.ext
  intro n
  exact ((W.packets Q xi).localRestriction n).trans
    ((C (characterWeightAt iota.prime Q (W.localEquiv Q xi))).localBrauer_reduction n)
...
def originalAutomorphismQuotientEquiv
  ...
  (W.packets Q xi).ambient.automorphismQuotientEquiv
...
def sourceHBlocks
  ...
  SporadicFi24P3Definition44NamedCarrierOriginalSourceHBlocks.sourceHBlockData (W.packets Q xi) H hHD hbaseD
...
end ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierOriginalDefinition41
