...
namespace ModularRep.PaperProofs.TypeCProposition313CriterionCorrespondence
...
structure SourceInputs where
  ...
  burnside : ∀ block : LiteralPrimitiveBlock k M,
    PublishedBurnsideMarkInjectivity.{0, 0}
      (A := MulAction.stabilizer
        (ActingGroup (k := k) G field hinvariant) block)
  labelToWeight : TypeCProposition311SourceInstantiation.OrdinarySeriesCarrier series ≃
  ...
  label_tensor : ∀ (lambda : TensorCharacters (k := k) G)
    ...
    labelToWeight
        (TypeCProposition313SourceInstantiation.ordinaryTensorStep
  ...
  label_field : ∀ (e : E)
      (x : TypeCProposition311SourceInstantiation.OrdinarySeriesCarrier series),
    labelToWeight
        (TypeCProposition313SourceInstantiation.ordinaryFieldStep
...
def globalCorrespondence
  ...
  have hexists :=
    TypeCProposition313SourceInstantiation.proposition_3_13_condition_ii_source_instantiated
...
end ModularRep.PaperProofs.TypeCProposition313CriterionCorrespondence
