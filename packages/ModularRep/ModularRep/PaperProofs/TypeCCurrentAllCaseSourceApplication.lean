import ModularRep.PaperProofs.TypeCCurrentFiniteSplitting

/-!
The current coefficient interface replaces only the higher odd and higher
even records. Every numerical branch, original prime divisor, fixed cover
and actual projective symplectic coordinate is retained. Historical wrappers
remain separate; their incompatible coefficient conjunctions are not used.
...
namespace ModularRep.PaperProofs.TypeCCurrentAllCaseSourceApplication
...
def CaseInputs (c : NumericalCase n p ell (Nat.card F)) : Type 1 := by
  ...
  | oddHigherRank _ _ _ _ _ =>
      exact TypeCCurrentFiniteSplitting.HigherOddInputs n ell F
  ...
  | evenHigherRank _ _ _ _ _ _ =>
      exact TypeCCurrentFiniteSplitting.HigherEvenInputs n ell F
...
def target (c : NumericalCase n p ell (Nat.card F))
  ...
  | oddHigherRank _ _ _ _ _ =>
      exact TypeCCurrentFiniteSplitting.HigherOddInputs.target n ell F D
  ...
  | rankThreeEven _ characteristic _ _ _ _ _ =>
      ...
      exact LowEvenInputs.target n ell F prime D
  | evenHigherRank _ _ _ _ _ _ =>
      exact TypeCCurrentFiniteSplitting.HigherEvenInputs.target n ell F D
...
theorem complete_at_case (c : NumericalCase n p ell (Nat.card F))
  ...
  | oddHigherRank _ _ _ _ _ =>
      exact TypeCCurrentFiniteSplitting.HigherOddInputs.complete n ell F D
  | sp6Two rank characteristic fieldTwo _ nondefining _ =>
      subst n
  ...
  | evenHigherRank rank _ _ _ _ _ =>
      exact TypeCCurrentFiniteSplitting.HigherEvenInputs.complete n ell F D rank
...
end ModularRep.PaperProofs.TypeCCurrentAllCaseSourceApplication
