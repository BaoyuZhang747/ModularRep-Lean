...
/-!
# One normalized Fischer correspondence with all named matched pairs
...
The principal positive branch is included. At Q=1, the chosen ambient
...
namespace ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierFi24Join
...
theorem exists_normalized_correspondence_with_named_pairs
      ...
      ∃ hOmega : ∀ (alpha : (MulAut X)ᵐᵒᵖ) (phi : IBr iota),
        ...
        ∃ hblock : ∀ phi, R.1.weightBlock (Omega phi) = brauerBlock iota hinj blocks phi,
          ...
          Nonempty (∀ phi : IBr iota,
            ...
            let M := matchAt iota hinj R localReduction Omega hOmega hblockOps phi
  ...
  let P := problemAt iota hinj R localReduction phi
  let M := matchAt iota hinj R localReduction Omega hOmega hblockOps phi
  exact exists_normalized_named_pair
    iota hinj R localReduction (operationsBlock iota hinj R phi) P rfl M
    (fi24ThreeBlockSourceOfBindings blocks Involution
...
end ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierFi24Join
