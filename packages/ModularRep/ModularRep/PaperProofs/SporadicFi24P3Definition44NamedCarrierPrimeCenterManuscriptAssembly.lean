import ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierPrimeCenterJointAssembly
import ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierFullCoverOuterTransport
import ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierGlobalQOneNormalization
import ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierJointSectorAssembly
import ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierCentralQuotientCorrespondenceTransport

/-! The centre-three manuscript disjoint union, with exact ordinary and joint ambient witnesses. -/
noncomputable section
set_option maxHeartbeats 4000000
namespace ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierPrimeCenterManuscriptAssembly
...
variable (singleton : DefectZeroReductionBlockSingleton (quotientRoot iota (Subgroup.center X)) OD)
...
include hthree hq hOuter principle S9295 S9495 S820 fieldSource singleton seedProper ambient localInputs hBlockD hD tau hinverts in
omit [Invertible (Fintype.card (Subgroup.center X) : k)] in
theorem prime_center_manuscript_deduction
  ...
  let routine : RoutineTransportInput iota inj bs R := ⟨⟩
  let hp : (Nat.card (Subgroup.center X)).Prime := by rw [hthree]; decide
  have hOuterX : Nat.card (LiteralOuterQuotient X) = 2 :=
    (fullCoverLiteralOuter_card (QuotientGroup.mk' (Subgroup.center X)) hq
      (QuotientGroup.ker_mk' (Subgroup.center X))).trans hOuter
  obtain ⟨e, hFamily, hIndependent, hFixed, hAD3, hBlocks, hQOne, hCommon, hCommonBlocks, hModels⟩ :=
    faithful_manuscript_intermediate_assembly iota R hOuterX tau (Or.inl hthree) hinverts
      compatU (fun Q theta => ⟨CU Q theta⟩) Dzero Tzero nu0 Bzero e0 hblock he0
  let E := physicalJoin iota R hprimeTo OD CU CD compatU compatD Sglobal Slocal hp eD e
  have hReduced := central_quotient_brauer_blocks_deduction iota R (Subgroup.center X) le_rfl hprimeTo
    OD CU CD compatU compatD Sglobal Slocal rfl hq eD hD hOuter principle hBlockD
    S9295 S9495 S820 fieldSource singleton seedProper ambient localInputs
  have ⟨hPartition, hJoint⟩ := joined_pair_of_retained_rows iota R hprimeTo OD CU CD compatU compatD
    Sglobal Slocal hp eD hq hD Dzero Tzero e hFamily hAD3 hModels hQOne hReduced
  have hbF : ∀ phi : FaithfulIBr iota inj bs R routine,
      R.1.weightBlock (e phi).val = brauerBlock iota inj bs phi.val :=
    fun phi => (hBlocks.1 phi).trans (operationsBlock_eq iota inj R bs phi.val)
  have ⟨hTrivial, hFaithful, hE, hBlock, hSector⟩ := physical_join_laws iota R hprimeTo OD CU CD compatU compatD
    Sglobal Slocal hp eD hq hBlockD hD e hFamily.1 hbF
  have hOne : GlobalQOneOutput iota E Dzero Tzero :=
    global_qOne_of_canonical_blocks iota E Dzero Tzero inj R
      (fun V => CU ⟨V.subgroup, V.radical⟩ ⟨V.localCharacter, V.defectZero⟩)
      (fun phi => (hBlock phi).trans (operationsBlock_eq iota inj R bs phi).symm) compatU Bzero
  refine ⟨e, hFamily, hIndependent, hFixed, hAD3, hBlocks, hQOne, hCommon, hCommonBlocks, hModels,
    hPartition, ?_, hReduced, hTrivial, hFaithful, hE, hBlock, hSector,
    ⟨(Equiv.sigmaFiberEquiv (fun phi : IBr iota => radicalClass (E phi))).symm⟩, ?_, ?_, hOne, hJoint, ?_⟩
  · exact SporadicFi24P3Definition44NamedCarrierCentralQuotientCorrespondenceTransport.central_quotient_correspondence_transport_deduction
      iota R (Subgroup.center X) le_rfl hprimeTo OD CU CD compatU compatD Sglobal Slocal eD hBlockD hD
  · intro Q
    refine ⟨(SporadicFi24P3Definition44NamedCarrierRepresentativeMaps.localMap iota E iota.prime Q).bijective,
      SporadicFi24P3Definition44NamedCarrierRepresentativeMaps.localMap_class iota E iota.prime Q, ?_⟩
    intro b
    exact ⟨SporadicFi24P3Definition44NamedCarrierRepresentativeMaps.blockLocalMap iota E R.1
      (brauerBlock iota inj bs) hBlock iota.prime Q b⟩
  · exact SporadicFi24P3Definition44NamedCarrierRepresentativeMaps.localMap_covariance iota E hE iota.prime
  · exact joint_sector_assembly iota inj bs routine
      (CentralKernelJointPair iota R hprimeTo CU CD eD hq hD) E hE hBlock hJoint

end ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierPrimeCenterManuscriptAssembly
