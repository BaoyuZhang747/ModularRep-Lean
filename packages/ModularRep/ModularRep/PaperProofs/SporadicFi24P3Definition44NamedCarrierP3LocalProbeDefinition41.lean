import ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierP3FullOrdinaryDefinition41
import ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierP3LocalProbeAction

/-! Full prime-three conclusion with the local fixed count derived from one
separating character probe. The same four actual rows need only exhaustivity;
their injectivity and corrected local action follow from eight literal values
and one group conjugacy binding. The specified probe/action binding is open. -/

noncomputable section
set_option maxHeartbeats 4000000
open scoped MonoidAlgebra
namespace ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierP3LocalProbeDefinition41
...
include AD Bridge hOuter ordinaryComplete ordinaryValues
  fusion support rowSurjective S414 evaluation stable hz probeValues imageProbeValues localFusion
  availability compatibility hDefect C allocation

theorem p3_definition41_from_local_probe
  ...
  exact SporadicFi24P3Definition44NamedCarrierP3FullOrdinaryDefinition41.p3_definition41_from_full_ordinary_table
    ...
    (fusion := fusion) (Q := Q) (support := support) (rows := rows)
    (rowBijective := ⟨local_rows_injective Q rows probe z hz probeValues, rowSurjective⟩)
    (S414 := S414) (evaluation := evaluation) (g := g) (stable := stable)
    (localFixed := local_fixed_card_two Q rows rowSurjective
      (localAut Q.1 (tau * MulAut.conj g) stable) probe imageProbe z hz
      probeValues imageProbeValues localFusion)
    (availability := availability) (compatibility := compatibility)
    (C := C) (allocation := allocation) (hDefect := hDefect) hcenter

end ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierP3LocalProbeDefinition41
