...
namespace ModularRep.PaperProofs.TypeBCurrentJordanQuotient
...
def gammaQuotient : M Frob.toMonoidHom Lbar →*
    SpecialClifford n F ⧸ SpinSubgroup n F N :=
  (QuotientGroup.mk' (SpinSubgroup n F N)).comp (gammaEmbedding points Lbar)

@[simp] theorem gammaQuotient_value (x : M Frob.toMonoidHom Lbar) :
    gammaQuotient points Lbar x =
      QuotientGroup.mk' (SpinSubgroup n F N) (gammaEmbedding points Lbar x) := rfl

theorem gammaQuotient_kernel
    (levi_le_spin : Lbar ≤ SpinSubgroup n A Nbar)
    (intersection : pairedLevi Lbar ⊓ SpinSubgroup n A Nbar ≤ Lbar) :
    (gammaQuotient points Lbar).ker = leviInGamma (Frob := Frob) Lbar := by
  ext x
  change (gammaEmbedding points Lbar x : SpecialClifford n F ⧸ SpinSubgroup n F N) = 1 ↔
    x.val.val ∈ Lbar
  rw [QuotientGroup.eq_one_iff,
    ← inclusion_spin_iff n p f F A N Nbar Frob points,
...
theorem gammaQuotient_surjective
  ...
  refine ⟨m, ?_⟩
  change QuotientGroup.mk' (SpinSubgroup n F N) (gammaEmbedding points Lbar m) =
    QuotientGroup.mk' (SpinSubgroup n F N)
      ((g : SpecialClifford n F) * gammaEmbedding points Lbar m)
  have hg : QuotientGroup.mk' (SpinSubgroup n F N) (g : SpecialClifford n F) = 1 :=
    (QuotientGroup.eq_one_iff (g : SpecialClifford n F)).mpr g.property
...
@[simp] theorem diagonalQuotientEquiv_value
    ...
    diagonalQuotientEquiv points Lbar levi_le_spin intersection product
      ...
      QuotientGroup.mk' (SpinSubgroup n F N) (gammaEmbedding points Lbar m) := rfl

end ModularRep.PaperProofs.TypeBCurrentJordanQuotient
