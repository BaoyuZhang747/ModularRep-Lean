...
covering block at index two in odd characteristic. The supported trace of
the selected idempotent proves this choice without a constituent source. -/

noncomputable section
open scoped MonoidAlgebra

namespace ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierSignedCoveringExtension
...
theorem exists_extension_in_covering_block_of_index_two
    ...
    ∃ selected : BrauerCharacterExtensionWitness iotaG iotaN phi,
      irreducibleBrauerCharacterBlock iotaG injG blocksG selected.val = B := by
  classical
  ...
  · have hscalar : catalogueG.centralCharacter bT e =
    ...
    refine ⟨selectedSign, ?_⟩
    change bT = B
    by_contra hne
    exact h1 (hscalar.symm.trans (catalogueG.centralCharacter_other hne))

end ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierSignedCoveringExtension
