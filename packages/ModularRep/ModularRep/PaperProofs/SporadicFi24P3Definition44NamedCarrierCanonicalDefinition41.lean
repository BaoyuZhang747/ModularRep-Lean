...
/-!
# A centreless presentation of Spath Definition 4.1 with canonical reductions
...
namespace ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierCanonicalDefinition41
...
structure Definition41Witness (Cover : EllPrimeCoverSource p X)
  ...
  packets : ∀ (Q : RadicalSubgroup (p := p) (G := X)) (xi : BrauerAtRadical iota Omega Q),
    PacketAt iota hinj R (canonicalSelectedReduction iota R C) Omega Q xi
...
variable (W : Definition41Witness iota hinj R C Cover hcenter D T)
...
/-- The packet has the same actual local character values. Specified blocks
are selected in the fixed canonical convention in the preceding theorem. -/
theorem packet_localBrauer_eq_canonical
    (Q : RadicalSubgroup (p := p) (G := X)) (xi : BrauerAtRadical iota W.Omega Q) :
    (localBrauer (W.packets Q xi)).1 =
      (C (characterWeightAt iota.prime Q (W.localEquiv Q xi))).localBrauer.1 := by
  apply PrimeRegularClassFunction.ext
  intro n
  exact (localBrauer_is_actual_reduction (W.packets Q xi) n).symm.trans
...
def originalAutomorphismQuotientEquiv
  ...
  (W.packets Q xi).ambient.automorphismQuotientEquiv.trans
    (quotientStabilizerEquiv (problemAt iota hinj R (canonicalSelectedReduction iota R C) xi.1) hcenter
      (ownBrauerAt iota hinj R (canonicalSelectedReduction iota R C) xi.1)
      (ownBrauerAt iota hinj R (canonicalSelectedReduction iota R C) xi.1) (W.packets Q xi).quotient).symm
...
def ofNormalizedFamily
    ...
    (family : ∀ phi : IBr iota,
      ...
      let M := matchAt iota hinj R (canonicalSelectedReduction iota R C) Omega hOmega hblock phi
      NormalizedNamedPairWitness P M S hcenter M.theta haut) :
    Definition41Witness iota hinj R C Cover hcenter D T where
  Omega := Omega
  equivariant := hOmega
  ...
  packets := fun Q xi => Classical.choice
    (exists_packetAt_of_normalized_family iota hinj R (canonicalSelectedReduction iota R C)
      Omega hOmega hblock S hcenter haut family Q xi)

end ModularRep.PaperProofs.SporadicFi24P3Definition44NamedCarrierCanonicalDefinition41
